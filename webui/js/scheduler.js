...
const fullComponentImplementation = function () {
  return {
    ...
    async fetchTasks() {
      ...
      try {
        ...
        if (!data || !data.tasks) {
          ...
          console.error(
            "Invalid response: data.tasks is not an array",
            data.tasks
          );
          this.tasks = [];
        } else {
          ...
          if (validTasks.length !== data.tasks.length) {
            console.warn(
              `Filtered out ${
                data.tasks.length - validTasks.length
              } invalid tasks`
            );
          }
    ...
    formatPlan(task) {
      ...
      const todoCount = Array.isArray(task.plan.todo)
        ? task.plan.todo.length
        : 0;
      const inProgress = task.plan.in_progress ? "Yes" : "No";
      const doneCount = Array.isArray(task.plan.done)
        ? task.plan.done.length
        : 0;
      ...
      if (Array.isArray(task.plan.todo) && task.plan.todo.length > 0) {
        try {
          ...
          if (!isNaN(nextTime.getTime())) {
            ...
            console.warn(
              `Invalid date format in plan.todo[0]: ${task.plan.todo[0]}`
            );
          }
        } catch (error) {
    ...
    formatSchedule(task) {
      ...
      if (typeof task.schedule === "string") {
        ...
        schedule = `${task.schedule.minute || "*"} ${
          task.schedule.hour || "*"
        } ${task.schedule.day || "*"} ${task.schedule.month || "*"} ${
    ...
    async refreshProjectOptions() {
      try {
        if (
          ...
          !projectsStore.projectList.length
        ) {
    ...
    async startCreateTask() {
      ...
      document
        .querySelector('[x-data="schedulerSettings"]')
        ?.setAttribute("data-editing-state", "creating");
      ...
      this.editingTask = {
        ...
        plan: {
          // Initialize plan for all task types to prevent undefined errors
          todo: [],
          in_progress: null,
          done: [],
        },
      ...
      this.selectedProjectSlug =
        initialProject && initialProject.name ? initialProject.name : "";
    ...
    async startEditTask(taskId) {
      ...
      document
        .querySelector('[x-data="schedulerSettings"]')
        ?.setAttribute("data-editing-state", "editing");
      ...
      const projectColor =
        task.project_color || (task.project ? task.project.color : "") || "";
      this.editingTask.project =
      ...
      this.selectedProjectSlug =
        this.editingTask.project && this.editingTask.project.name
          ? this.editingTask.project.name
          : "";
      ...
      if (
        ...
        typeof this.editingTask.schedule === "string"
      ) {
        let scheduleObj = {
          minute: "*",
          hour: "*",
      ...
      if (this.editingTask.type === "scheduled") {
        ...
        if (!this.editingTask.token) {
          ...
          console.log(
            "Generated new token for adhoc task:",
            this.editingTask.token
          );
        }
    ...
    cancelEdit() {
      ...
      this.editingTask = {
        ...
        token: "",
        plan: {
          // Initialize plan for planned tasks
          todo: [],
          in_progress: null,
          done: [],
        },
      ...
      document
        .querySelector('[x-data="schedulerSettings"]')
        ?.removeAttribute("data-editing-state");
    },
    ...
    async saveTask() {
      ...
      if (!this.editingTask.name.trim() || !this.editingTask.prompt.trim()) {
        ...
        alert(
          window.i18n?.translate("scheduler.task_required") ||
            "Task name and prompt are required"
        );
        return;
      }
      ...
      try {
        ...
        if (this.editingTask.type === "scheduled") {
          ...
          console.log(
            "Planned task plan data:",
            JSON.stringify(taskData.plan, null, 2)
          );
    ...
    async runTask(taskId) {
      try {
        ...
        const toastMessage =
          data.warning || data.message || "Task started successfully";
    ...
    async deleteTask(taskId) {
      ...
      if (
        !confirm(
          window.i18n?.translate("scheduler.confirm_delete") ||
      ...
      try {
        ...
        if (
          ...
          this.selectedTaskForDetail.uuid === taskId
        ) {
    ...
    get filteredTasks() {
      ...
      if (this.filterType && this.filterType !== "all") {
        filtered = filtered.filter((task) => {
          ...
          return (
            String(task.type).toLowerCase() === this.filterType.toLowerCase()
          );
        });
      }
      ...
      if (this.filterState && this.filterState !== "all") {
        filtered = filtered.filter((task) => {
          ...
          return (
            String(task.state).toLowerCase() === this.filterState.toLowerCase()
          );
        });
      }
    ...
    debugTasks() {
      ...
      if (!Array.isArray(this.tasks)) {
        console.error(
          "ERROR: this.tasks is not an array!",
          typeof this.tasks,
          this.tasks
        );
        console.groupEnd();
      ...
      if (this.tasks.length > 0 && filteredTasks.length === 0) {
        ...
        if (this.filterType !== "all") {
          const typeMatch = this.tasks.some(
            (t) =>
              t.type &&
              String(t.type).toLowerCase() === this.filterType.toLowerCase()
          );
          console.log(`Type "${this.filterType}" matches found:`, typeMatch);
        }

        if (this.filterState !== "all") {
          const stateMatch = this.tasks.some(
            (t) =>
              t.state &&
              String(t.state).toLowerCase() === this.filterState.toLowerCase()
          );
      ...
      const hasUndefinedType = this.tasks.some(
        (t) => t.type === undefined || t.type === null
      );
      const hasUndefinedState = this.tasks.some(
        (t) => t.state === undefined || t.state === null
      );
    ...
    updateTasksUI() {
      ...
      this.$nextTick(() => {
        ...
        const hasFilteredTasks =
          Array.isArray(this.filteredTasks) && this.filteredTasks.length > 0;
...
if (window.Alpine) {
  ...
  document.addEventListener("alpine:init", () => {
    console.log(
      "Alpine:init - immediately registering schedulerSettings component"
    );
  ...
  const setupSchedulerTab = () => {
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        const schedulerTab = e.target.closest(
          ...
          setTimeout(() => {
            ...
            if (schedulerElement) {
              ...
              if (typeof schedulerData.fetchTasks === "function") {
                ...
                console.error(
                  "fetchTasks is not a function on scheduler component"
                );
              }
              ...
              if (typeof schedulerData.startPolling === "function") {
                ...
                console.error(
                  "startPolling is not a function on scheduler component"
                );
              }
            } else {

const settingsModalProxy = {
  ...
  get filteredSections() {
    ...
    const filteredSections = this.settings.sections.filter(
      (section) => section.tab === this.activeTab
    );
  ...
  switchTab(tabName) {
    ...
    setTimeout(() => {
      ...
      if (tabName === "scheduler") {
        ...
        const schedulerElement = document.querySelector(
  ...
  async openModal() {
    ...
    try {
      ...
      const settings = {
        title: window.t ? window.t("settings.title", "Settings") : "Settings",
        buttons: [
          {
            id: "save",
            title: window.t ? window.t("common.save", "Save") : "Save",
            classes: "btn btn-ok",
          },
          {
            id: "cancel",
            title: window.t ? window.t("common.cancel", "Cancel") : "Cancel",
            type: "secondary",
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          // Debug log
          const schedulerTab = document.querySelector(
            '.settings-tab[title="Task Scheduler"]'
          );
          console.log(
            `Current active tab after direct set: ${modalAD.activeTab}`
          );
          console.log(
          ...
          if (modalAD.activeTab === "scheduler") {
            console.log(
              "Settings opened directly to scheduler tab, initializing polling"
            );
            const schedulerElement = document.querySelector(
              '[x-data="schedulerSettings"]'
            );
            if (schedulerElement) {
              ...
              if (
                schedulerData &&
                typeof schedulerData.startPolling === "function"
              ) {
                schedulerData.startPolling();
      ...
      if (schedulerComponent) {
        ...
        observer.observe(schedulerComponent, {
          attributes: true,
          subtree: true,
          childList: true,
        });
  ...
  async handleButton(buttonId) {
    if (buttonId === "save") {
      ...
      // Check for language changes and update i18n immediately
      console.log("Checking for language changes in saved settings...");
      console.log("Modal settings:", modalAD.settings);

      if (window.i18n && modalAD.settings) {
        // Try to find ui_language in the flat settings object
        let newLang = modalAD.settings.ui_language;

        // If not found in flat structure, look in sections
        if (!newLang && modalAD.settings.sections) {
        ...
        console.log("Found language setting:", newLang);
        console.log("Current i18n language:", window.i18n.getCurrentLanguage());

        if (newLang && newLang !== window.i18n.getCurrentLanguage()) {
        ...
        console.warn("i18n system or settings not available");
      }

      document.dispatchEvent(
        new CustomEvent("settings-updated", { detail: resp.settings })
      );
      this.resolvePromise({
        status: "saved",
        data: resp.settings,
  ...
  watchLanguageChanges() {
    console.log("Setting up language change watchers...");
    ...
    document.addEventListener("settings-updated", (event) => {
      ...
      if (settings && settings.sections) {
        const uiSection = settings.sections.find(
          (section) => section.id === "ui_settings"
        );
        if (uiSection) {
          const langField = uiSection.fields.find(
            (field) => field.id === "ui_language"
          );
          if (
            langField &&
            window.i18n &&
            langField.value !== window.i18n.getCurrentLanguage()
          ) {
            console.log(
              "Language change detected via settings event:",
              langField.value
            );
            window.i18n.setLanguage(langField.value);
          }
        }
      }
    });

    // Also watch for direct field changes in the settings modal
    const watchSettingsFields = () => {
      // Find all select elements that might be language selectors
      const selects = document.querySelectorAll("#settingsModal select");
      selects.forEach((select) => {
        // Check if this select is for ui_language by looking at nearby labels or field structure
        const fieldContainer = select.closest(".field");
        if (fieldContainer) {
          const label = fieldContainer.querySelector(".field-title");
          if (
            label &&
            (label.textContent.includes("Language") ||
              label.textContent.includes("语言"))
          ) {
            console.log("Found language selector, adding change listener");
            select.addEventListener("change", (event) => {
              const newLang = event.target.value;
              console.log("Language selector changed to:", newLang);
              if (window.i18n && newLang !== window.i18n.getCurrentLanguage()) {
                window.i18n.setLanguage(newLang);
              }
            });
          }
    ...
    // Watch for settings modal opening and set up field watchers
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === "childList") {
          const settingsModal = document.getElementById("settingsModal");
          if (settingsModal && settingsModal.style.display !== "none") {
            setTimeout(watchSettingsFields, 500); // Wait for Alpine.js to render
          }
        }
      });
    });
...
document.addEventListener("alpine:init", function () {
  ...
  // Initialize language change watching
  if (settingsModalProxy.watchLanguageChanges) {
    settingsModalProxy.watchLanguageChanges();
  }
    ...
    return {
      ...
      updateFilteredSections() {
        ...
        if (this.activeTab === "agent") {
          this.filteredSections =
            this.settingsData.sections?.filter(
              (section) => section.tab === "agent"
            ) || [];
        } else if (this.activeTab === "external") {
          this.filteredSections =
            this.settingsData.sections?.filter(
              (section) => section.tab === "external"
            ) || [];
        } else if (this.activeTab === "developer") {
          this.filteredSections =
            this.settingsData.sections?.filter(
              (section) => section.tab === "developer"
            ) || [];
        } else if (this.activeTab === "mcp") {
          this.filteredSections =
            this.settingsData.sections?.filter(
              (section) => section.tab === "mcp"
            ) || [];
        } else if (this.activeTab === "backup") {
          this.filteredSections =
            this.settingsData.sections?.filter(
              (section) => section.tab === "backup"
            ) || [];
        } else {
          ...
          for (const section of this.settingsData.sections) {
            for (const field of section.fields) {
              if (
                ...
                showToast(
                  `${field.title} in ${section.title} is required`,
                  "error"
                );
                return;
              }
      ...
      closeModal() {
        ...
        if (schedulerElement) {
          ...
          if (
            schedulerData &&
            typeof schedulerData.stopPolling === "function"
          ) {
            console.log("Stopping scheduler polling on modal close");
            schedulerData.stopPolling();
          }
